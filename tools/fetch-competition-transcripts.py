...
# GAME_RE is used to parse the game transcript from the HTML page.
#
# Example from https://www.codecup.nl/showgame.php?ga=313601
#
# <script type="application/javascript">let g = new ZeroPointOne(["Maks Verver","Nikita Kurgin"], ["Start"].concat(["AWADFAANDAFDAADA","adaadfadnaafdawa","a6c4","h1f2","b2d4","f2e4","b4b2","g3e5","a7c5","e5c3","d4f6","h8f6","c5e7","c3a5","b5d7","ac6","d7f5","h5f5","a4a6","c6a8","e7g5","h4g5","c4e6","g4e6","Dc7","ac4","b3c4","e6c4","Ae6","c4a2"]));</script><br>
#
# I want to extract the player names (to verify I have the right game), and the
# list of moves played.
GAME_RE = re.compile(r'<script type="application/javascript">let g = new ZeroPointOne\((\[.*?]), \["Start"].concat\((\[.*?])\)\);')
    ...
    for i, row in enumerate(csv.reader(f)):
      if i == 0:
        ...
        if player == PLAYER_NAMES[0]:
          ...
          game.user1 = user
          game.score1 = int(score)
          game.status1 = status
        elif player == PLAYER_NAMES[1]:
          assert game.user2 is None
          game.user2 = user
          game.score2 = int(score)
          game.status2 = status
        else:
...
def WriteAnnotatedCsv(filename, games):
  with open(filename, 'wt', newline='') as f:
    ...
    w.writerow(('Game', 'Round', 'IsSwiss', 'User1', 'Score1', 'Status1', 'User2', 'Score2', 'Status2', 'Moves'))
    for game in games:
      w.writerow((game.game_id, game.round, game.is_swiss, game.user1, game.score1, game.status1, game.user2, game.score2, game.status2, game.moves))
